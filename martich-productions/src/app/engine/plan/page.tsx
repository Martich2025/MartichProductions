...
export default function MiniPlanPage() {
  ...
  useEffect(() => {
    ...
    try {
        ...
        ;(async () => {
          try {
            ...
            if (j?.ok) {
              ...
              let sessionChoices = {} as MiniPlan['choices']
              try {
                sessionChoices = JSON.parse(sessionStorage.getItem('engine_map_choices') || '{}') as MiniPlan['choices']
              } catch (e) {
          ...
          try {
            choices = JSON.parse(sessionStorage.getItem('engine_map_choices') || '{}')
            form = JSON.parse(sessionStorage.getItem('engine_map_form') || '{}')
            planData = JSON.parse(sessionStorage.getItem('engine_mini_plan') || '{}')
            auditData = JSON.parse(sessionStorage.getItem('engine_mini_plan_audit') || '{}')
          } catch (e) {
      ...
      let current = {} as MiniPlan
      try {
        current = JSON.parse(sessionStorage.getItem('engine_mini_plan') || '{}') as MiniPlan
      } catch (e) {
        console.warn('Failed to read engine_mini_plan from sessionStorage:', e)
      }
      ...
      <div className="min-h-screen bg-canvas text-primary">
        <section className="py-12 sm:py-14 md:py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
            ...
            <div className="text-tertiary mb-4">A fast, visual outline of the first 90 days—hooks, cadence, and site fixes—so you know exactly what we’ll build and why it converts.</div>
            {plan ? (
              <div className="space-y-6">
                {audit && (
                  <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative w-14 h-14">
                          <svg viewBox="0 0 36 36" className="w-14 h-14">
                            <path className="text-tertiary" strokeWidth="4" stroke="currentColor" fill="none" d="M18 2a16 16 0 1 1 0 32 16 16 0 0 1 0-32" />
                            <path className="text-brand" strokeWidth="4" strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2a16 16 0 1 1 0 32 16 16 0 0 1 0-32" style={{ strokeDasharray: `${(audit.score || 0) * 0.01 * 100}, 100` }} />
                          </svg>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-tertiary">Higher = stronger funnel</div>
                        </div>
                      </div>
                      {/* KPI */}
                      <div>
                        <div className="text-sm text-secondary mb-1">CTAs: {(audit?.site?.ctas || []).slice(0,3).join(', ') || '—'}</div>
                        <div className="text-sm text-secondary mb-1">Proof: {audit?.site?.hasProofSignals ? 'Present' : 'Missing'}</div>
                        <div className="text-sm text-secondary">Canonical: {audit?.site?.canonical ? 'Present' : 'Missing'}</div>
                        {typeof audit.targetScore === 'number' && (
                          <div className="text-sm text-secondary">Opportunity: +{Math.max(0, (audit.targetScore || 0) - (audit.score || 0))} → target {audit.targetScore}</div>
                        )}
                        {typeof audit.confidence === 'number' && (
                          <div className="text-sm text-secondary">Confidence: {Math.round((audit.confidence || 0) * 100)}%</div>
                        )}
                      </div>
                      {/* Social Reach */}
                      <div>
                        <div className="text-sm text-secondary mb-1">IG: {audit.socialsMeta?.instagram?.followersApprox ?? '—'}</div>
                        <div className="text-sm text-secondary mb-1">FB: {audit.socialsMeta?.facebook?.followersApprox ?? '—'}</div>
                        <div className="text-sm text-secondary">YT: {audit.socialsMeta?.youtube?.subscribersApprox ?? '—'}</div>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="p-3 rounded-xl border border-subtle bg-canvas/40">
                    ...
                    <div className="flex gap-1" aria-label="Issue heat strip">
                      {audit.issues.slice(0, 24).map((iss, i) => (
                        <div key={i} className={`h-2 flex-1 rounded ${iss.severity === 'high' ? 'bg-danger' : iss.severity === 'medium' ? 'bg-warning' : 'bg-brand'}`} title={`${iss.title} — ${iss.severity}`} />
                      ))}
                    </div>
                ...
                {audit && (
                  <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                    ...
                    <div className="text-sm text-secondary">Links: {audit.site?.ecosystem?.linksTotal ?? '—'} ({audit.site?.ecosystem?.linksExternal ?? 0} external)</div>
                    <div className="text-sm text-secondary">Images: {audit.site?.ecosystem?.images ?? '—'}</div>
                    <div className="text-sm text-secondary">Videos: {audit.site?.ecosystem?.videos ?? '—'}</div>
                  </div>
                )}
                {audit && (
                  <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                    <div className="font-semibold mb-2">Accessibility Snapshot</div>
                    <div className="text-sm text-secondary">H1 Count: {audit.site?.a11y?.h1Count ?? '—'}</div>
                    <div className="text-sm text-secondary">Images missing alt: {audit.site?.a11y?.imagesMissingAlt ?? '—'}</div>
                  </div>
                )}
                ...
                {audit && (
                  <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        className="text-xs px-2 py-1 rounded border border-mp-gray-700 text-secondary hover:border-mp-gold"
                        onClick={() => {
                    ...
                    <div className="text-sm text-secondary mb-2">Score: {Math.max(0, Math.min(100, audit.score || 0))}/100</div>
                    <div className="text-sm text-secondary mb-1">Top gaps:</div>
                    <ul className="text-secondary text-sm space-y-1">
                      {(audit.insights?.gaps || []).slice(0, 3).map((g, i) => <li key={i}>• {g}</li>)}
                    </ul>
                    <div className="text-sm text-secondary mt-2">Next 7 days:</div>
                    <ul className="text-secondary text-sm space-y-1">
                ...
                {audit && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                      ...
                      <div className="text-sm text-secondary">Site: {audit.domain || '—'}</div>
                      <div className="text-sm text-secondary">Instagram: {audit.socials?.instagram ? 'yes' : 'no'}</div>
                      <div className="text-sm text-secondary">Facebook: {audit.socials?.facebook ? 'yes' : 'no'}</div>
                      <div className="text-sm text-secondary">YouTube: {audit.socials?.youtube ? 'yes' : 'no'}</div>
                      {typeof audit.confidence === 'number' && (
                        <div className="text-sm text-secondary">Confidence: {Math.round((audit.confidence || 0) * 100)}%</div>
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 rounded-xl border border-subtle bg-canvas/40">
                    ...
                    <div className="text-secondary">{plan?.choices?.persona || '—'}</div>
                  </div>
                  <div className="p-4 rounded-xl border border-subtle bg-canvas/40">
                    <div className="font-semibold mb-1">Tone</div>
                    <div className="text-secondary">{plan?.choices?.tone || 'Elegant'}</div>
                  </div>
                </div>
                <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                  ...
                  <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {audit.insights.gaps.slice(0, 6).map((g, i) => (
                        <div key={i} className="p-3 rounded-lg border border-subtle bg-canvas/40 flex items-start justify-between gap-3">
                          <div className="text-sm text-secondary">{g}</div>
                          <button
                            className="text-xs px-2 py-1 rounded border border-mp-gray-700 text-secondary hover:border-mp-gold"
                ...
                <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {Array.from({ length: 12 }).map((_, w) => (
                      <div key={w} className="p-3 rounded-lg border border-subtle bg-canvas/40">
                        <div className="text-xs text-tertiary mb-1">Week {w + 1}</div>
                        <ul className="text-sm text-secondary space-y-1">
                ...
                <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                    {[
                      ...
                      <div key={x.w} className="p-3 rounded-lg border border-subtle bg-canvas/40">
                        <div className="text-xs text-tertiary mb-1">Week {x.w}</div>
                        <ul className="text-sm text-secondary space-y-1">
                ...
                <div className="p-3 sm:p-4 rounded-xl border border-subtle bg-canvas/40">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <div>
                      <div className="text-sm text-secondary">Reels/Shorts</div>
                      <div className="text-xs text-mp-gray-500">Top‑of‑funnel reach; weekly cadence</div>
                    </div>
                    <div>
                      <div className="text-sm text-secondary">Hero Film / YouTube</div>
                      <div className="text-xs text-mp-gray-500">Brand story + SEO; anchors the plan</div>
                    </div>
                    <div>
                      <div className="text-sm text-secondary">Site / Landing</div>
                ...
                <div className="p-4 rounded-xl border border-subtle bg-canvas/40">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <div>
                      <div className="text-xs text-tertiary mb-1">Day 30</div>
                      <ul className="text-sm text-secondary space-y-1">
                    ...
                    <div>
                      <div className="text-xs text-tertiary mb-1">Day 60</div>
                      <ul className="text-sm text-secondary space-y-1">
                    ...
                    <div>
                      <div className="text-xs text-tertiary mb-1">Day 90</div>
                      <ul className="text-sm text-secondary space-y-1">
              ...
              <div className="text-secondary">No plan found. <a className="text-brand underline" href="/engine/map">Start the engine map</a>.</div>
            )}
          </div>
        </section>
        {plan && !stickyDismissed && (
          <div className="fixed inset-x-0 bottom-0 z-30" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 0.5rem)' }}>
            <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
              <div className={`relative rounded-xl border border-subtle bg-mp-charcoal/90 backdrop-blur supports-[backdrop-filter]:bg-mp-charcoal/70 p-3 flex flex-col sm:flex-row items-center gap-3 shadow-lg ${pulseCta ? 'ring-2 ring-mp-gold/60' : ''}`} aria-label="Plan actions">
                <button
                  aria-label="Dismiss"
                  className="absolute top-2 right-2 text-mp-gray-500 hover:text-secondary"
                ...
                {plan && audit && (
                  <motion.div
                    ...
                    className="w-full grid grid-cols-3 text-center text-[11px] text-tertiary"
                  ...
                  >
                    <div className="py-1 border-b border-subtle">Hooks: {(plan?.hooks || []).length}</div>
                    <div className="py-1 border-b border-subtle">CTAs found: {(audit?.site?.ctas || []).length}</div>
                    <div className="py-1 border-b border-subtle">Proof: {audit?.site?.hasProofSignals ? 'Present' : 'Missing'}</div>
                  </motion.div>
                )}
                ...
                <div className="w-full sm:w-auto flex items-center gap-2">
                  <input
                    ...
                    className="h-9 px-3 rounded-md bg-canvas border border-subtle text-sm text-white placeholder:text-mp-gray-500 flex-1 sm:w-56"
                  ...
                  >
                    ...
                    {stickyEmailMsg && (
                  <div id="sticky-email-feedback" role="status" aria-live="polite" className={`w-full text-[11px] mt-1 ${stickyEmailMsg.type === 'success' ? 'text-success' : 'text-danger'}`}>
                    {stickyEmailMsg.text}
                  </div>
                )}
                <div className="w-full text-right">
                  <div className="text-[11px] text-mp-gray-500">
                    ...
                    <span className="ml-2">Prefer a producer call? <a href={`tel:${CONTACT_PHONE_TEL}`} onClick={() => analytics.customEvent('engine_producer_call', { from: 'plan_sticky' })} className="underline hover:text-brand">Call</a> or <a href={`mailto:${CONTACT_EMAIL}`} onClick={() => analytics.customEvent('engine_producer_email', { from: 'plan_sticky' })} className="underline hover:text-brand">email</a></span>
                  </div>
                </div>
              </div>
            </div>

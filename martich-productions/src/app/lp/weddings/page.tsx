      ...
      <div className="min-h-screen bg-canvas">
        ...
        <section className="py-16 bg-surface text-primary">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center max-w-3xl">
            ...
            <motion.p initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="text-xl text-secondary mb-8">Elegant, story‑driven coverage crafted with precision—so you can relive the day forever.</motion.p>
            <Button href="/book" size="lg" className="bg-brand text-canvas hover:bg-brand-600">Check Availability <ArrowRight className="ml-2 w-5 h-5"/></Button>
            <div className="text-sm text-tertiary mt-2">Reply within 24h • no obligation</div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={m.l} className="text-center bg-surface rounded-xl p-6 border border-subtle">
                  <div className="text-2xl font-bold text-brand tabular-nums mb-1">{m.v}</div>
                  <div className="text-xs uppercase text-secondary metric-label">{m.l}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {["Story‑Driven Films","Crystal‑Clear Audio","Luxury‑Grade Crew"].map((f) => (
                <div key={f} className="bg-surface rounded-xl p-6 border border-subtle">
                  ...
                  <div className="text-sm text-secondary">Meticulous capture, unobtrusive operation, on‑brand edits for social and film.</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 bg-canvas">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl text-center">
            <div className="text-brand font-bold mb-2">What couples love</div>
            <div className="text-secondary mb-6">Same‑day social edits • Full ceremony film • Multi‑cam speeches • Director + second shooter</div>
            <Button href="/book" size="lg" className="bg-brand text-canvas hover:bg-brand-600">Start My Date Hold <ArrowRight className="ml-2 w-5 h-5"/></Button>
          </div>
        </section>

        {/* Sticky mobile CTA */}
        <div className="md:hidden fixed inset-x-0 bottom-0 z-40 bg-surface/95 border-t border-subtle p-3 backdrop-blur-sm">
          <Button href="/book" className="w-full bg-brand text-canvas hover:bg-brand-600">Check Availability</Button>
        </div>
      </div>
    </MainLayout>
  )
